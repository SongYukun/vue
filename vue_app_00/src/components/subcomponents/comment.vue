  ...
  <div class="app_subcomment">
    ...
    <div class="cmt-list">
      <div class="cmt-list-item" v-for="(item,i) in list" :key="item.id">
        <div class="cmt-title">
          第{{i+1}}楼 &nbsp;&nbsp;用户名:{{item.username}}  发表时间:{{item.ctime | dateFormat}}
        </div>
        <div class="cmt-body">
          {{item.content}}
        </div>
    ...
    <mt-button size="large" type="danger" @click="getCommentList">加载更多</mt-button>
  </div>
</template>
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getCommentList(){
        ...
        // 2.点击加载更多按钮加载下一页        
        // 3.添加新评论，看第一条内容
        this.pageIndex++; //当前页加1
        var url="newslist/commentlist?nid="+this.id; //#url
